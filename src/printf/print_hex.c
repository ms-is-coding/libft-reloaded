	...
	int mask, int iters)
{
	if (!iters)
		return ;
	_ft_printf_hex_internal(p, n >> 4, mask, iters - 1);
	_ft_printf_insert(p, (char)("0123456789ABCDEF"[n & 0xF] | mask));
}
...
static int	_ft_printf_0x(t_printf_parser *p, int start, int mask)
{
	...
	if (!start && p->flags & PRINTF_FLAG_ZEROPAD)
		return (0);
	_ft_printf_insert(p, '0');
	_ft_printf_insert(p, (char)('X' | mask));
	return (2);
}

void	_ft_printf_hex(t_printf_parser *p, uintmax_t n, int mask)
{
	const int	len = _ft_printf_print_len(p, n);
	const int	off = ((p->flags & PRINTF_FLAG_ALTERNATE) > 0) << 1;

	if (p->prec >= 0)
		p->flags &= ~PRINTF_FLAG_ZEROPAD;
	if (n != 0)
		_ft_printf_0x(p, PRINTF_START, mask);
	_ft_printf_padding(p, len + off, PRINTF_START, PRINTF_NUMERIC);
	if (n != 0)
		_ft_printf_0x(p, PRINTF_END, mask);
	_ft_printf_hex_internal(p, n, mask, len);
	_ft_printf_padding(p, len + off, PRINTF_END, PRINTF_NUMERIC);
}
...
void	_ft_printf_pointer(t_printf_parser *p, const void *ptr)
{
	if (!ptr)
	{
		_ft_printf_str(p, "(nil)");
		return ;
	}
	p->flags |= PRINTF_FLAG_ALTERNATE;
	_ft_printf_hex(p, (size_t)ptr, 0x20);
}
